...
<center>
    ...
    <table class="table table-bordered table-hover table-striped" style="width: 50%">
        ...
        //执行查询并返回结果集
        $sql="select * from newsusers order by id";
        ...
        while($row =mysqli_fetch_assoc($result)){
            ...
            echo "<td style='text-align: center'><a href='UserModify.php?id={$row['id']}'>修改</a>&nbsp;&nbsp;| &nbsp;&nbsp;<a href='javascript:dodel({$row['id']})'>删除</a></td>";
            echo"</tr>";
            $cid = $row['id'];
        }
        ...
        <script>
            ...
            function dodel(id){
                if (confirm("确定要删除吗")){
                    window.location="action.php?action=userdel&cid="+id;
                }
            }
        </script>
    </table>
...
//                    $data2 = $_POST;
////注意set后面有一个空格
////                    $sql = "insert into newsusers set ";
//                    $sql2 = "insert into `newsusers` (`category`) VALUES ('4'); ";
...
//                    $link = require_once "conn.php";
                    ...
                    echo $sql;
                    if ($res) {
                        echo "<script>alert('数据添加成功！');top.location.href='UserManage.php'</script>";
                    } else {
